...
class PuzzleRepository(private val api: DailyApiService) {
    ...
    suspend fun submitSimpleGuess(puzzleId: String, guess: String, mode: GameMode): com.example.daily.domain.model.SimpleGuessResult? {
        val request = GuessRequest(puzzleId = puzzleId, guess = guess)
        val response = when (mode) {
            GameMode.DAILY -> api.submitDailyGuess(puzzleId, request)
            GameMode.INFINITE -> api.submitInfiniteGuess(puzzleId, request)
        ...
        if (!response.valid) return null
        return com.example.daily.domain.model.SimpleGuessResult(
            valid = response.valid,
            isCorrect = response.correct == true,
            message = response.message ?: "",
        )
    }

    suspend fun submitMissingLinkGuess(puzzleId: String, guess: String, mode: GameMode): com.example.daily.domain.model.MissingLinkGuessResult? {
        val request = GuessRequest(puzzleId = puzzleId, guess = guess)
        val response = when (mode) {
            GameMode.DAILY -> api.submitDailyGuess(puzzleId, request)
            GameMode.INFINITE -> api.submitInfiniteGuess(puzzleId, request)
        }
        if (!response.valid) return com.example.daily.domain.model.MissingLinkGuessResult(
            valid = false,
            isCorrect = false,
            theme = null,
            items = null,
        ...
        return com.example.daily.domain.model.MissingLinkGuessResult(
            valid = response.valid,
            isCorrect = response.correct == true,
